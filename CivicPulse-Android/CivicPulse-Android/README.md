...
3. Run on an emulator or device. Debug builds default to
   ...
   localhost); release builds default to `https://civicpulse-india.onrender.com/`.
...
- **Filed complaints persist locally and actually show up.** Since there's
  no real list/lookup endpoint yet, a docket you file would otherwise vanish
  the moment you leave the confirmation screen. `data/local/FiledComplaintsStore.kt`
  persists what you've filed on this device (DataStore + JSON), and
  `ComplaintRepository` merges it on top of the demo data everywhere — the
  dashboard's complaint list, the Track queue, and docket lookup. Dashboard
  and Track both re-pull when you navigate back to them, so a complaint you
  just filed appears immediately. Clearable from Account → Developer options.
